...
### Example starts here  - run  code above to have function available:
ddf<-data.frame(values = runif(100), 
           group = sample(LETTERS[1:5],100,T))

ggplot(ddf, aes(group, values,color=group,fill= group)) +
